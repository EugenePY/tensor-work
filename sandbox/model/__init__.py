...
class NNet_abstract(object):
    """ Connect all the input layer
    """
    def __init__(self, input_list, layer_list):
        self.input = input_list
        self.layers = layer_list
        self.params = []

    def build_nnet(self, layer_list):
        print "Stacking the Network"
        self.nnet_output = None
        for i, layer in enumerate(self.layers):
            if i == 0:
                input = self.input
            else:
                input = self.nnet_output

            self.nnet_output = layer.fprob(input)
...
class MCMC_abstract(object):
    def __init__(self, rng=None):
        if rng is None:
            rng = RandomStreams(2**22)
